![Banner](.github/assets/Banner.png)

## BETA VERSION

PixeUI.com is currently in beta stage, we are working hard to release a stable version. We will like to hear your feedback and suggestions.

## Documentation

Visit [**www.pixeui.com/docs**](https://www.pixeui.com/docs) to view the documentation.

## Code of Conduct

This project follows a Code of Conduct to help create a welcoming community.

Please read [**CODE_OF_CONDUCT.md**](./CODE_OF_CONDUCT.md) before contributing.

## License

Licensed under the [**MIT + Commons Clause License Condition**](./LICENSE.md).

## Links

- Website: [**https://www.pixeui.com**](https://www.pixeui.com)
- Documentation: [**https://www.pixeui.com/docs**](https://www.pixeui.com/docs)
- Components: [**https://www.pixeui.com/components**](https://www.pixeui.com/components)
- GitHub: [**https://github.com/Pixe-UI**](https://github.com/Pixe-UI)
- Discord: [**https://discord.gg/gNAHhRaCJD**](https://discord.gg/gNAHhRaCJD)

<br />
<a href="https://vercel.com/oss">
  <img alt="Vercel OSS Program" src="https://vercel.com/oss/program-badge.svg" />
</a>
